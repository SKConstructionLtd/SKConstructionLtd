...
export default function GalleryPage() {
  ...
  const [selectedImageIndex, setSelectedImageIndex] = useState<{ category: typeof categoryConfig[0], index: number } | null>(null);
  ...
  // Generate all image paths for a category
  const getCategoryImages = (category: typeof categoryConfig[0]) => {
    const images = [];
    const baseName = category.folder;
    for (let i = 1; i <= category.imageCount; i++) {
      images.push(`/images/${category.folder}/${baseName}-${i}.jpg`);
    }
    return images;
  };

  const handleImageClick = (category: typeof categoryConfig[0], index: number = 0): void => {
